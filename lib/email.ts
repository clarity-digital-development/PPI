...
// ─────────────────────── Installer dispatch email ───────────────────────
//
// Price-free "today's jobs" list for hourly install crews (Ryan, 2026-08-25).
// Built from lib/dispatch DTOs that carry no money fields at all — see
// lib/dispatch/types.ts for the three-allowlist design. This is deliberately
// NOT a flag on sendAdminOrderNotification: that template interleaves prices
// throughout, and one missed branch would leak a dollar amount to the crew.
//
// No shouldSendEmail() call: this is internal/admin mail, not customer mail
// (lib/email-preferences.ts), exactly like sendInvoiceEmail.
...
import {
  ...
  type InstallerDispatchEmailProps,
} from '@/lib/dispatch/types'

function dispatchDayLabel(day: string | null, style: 'long' | 'short'): string {
  if (!day) return 'Next available'
...
function dispatchLineText(l: { description: string; quantity: number }): string {
  // Descriptions usually embed their own "× 2"; only append when the stored
  // quantity is actually > 1 so the crew never reads "× 2 × 1".
  return l.quantity > 1 && !/×|\bx\s?\d/i.test(l.description) ? `${l.description} × ${l.quantity}` : l.description
}

function renderDispatchOrderText(j: DispatchOrderJob, n: number): string {
  const out: string[] = []
  out.push(`${n}) ${j.address.line1}, ${j.address.city} ${j.address.state} ${j.address.zip}${j.isExpedited ? '        ⚡ EXPEDITED' : ''}${j.status === 'completed' ? '   (already marked completed)' : ''}`)
  out.push(`   Map: ${j.mapsUrl}`)
  out.push(`   Date: ${dispatchDayLabel(j.scheduledDate, 'long')}`)
  out.push(`   Agent: ${j.agent.name}${j.agent.phone ? ` (${j.agent.phone})` : ''}${j.agent.company ? ` · ${j.agent.company}` : ''}`)
  if (j.soldBy) out.push(`   Sold by: ${j.soldBy.name}${j.soldBy.phone ? ` (${j.soldBy.phone})` : ''}`)
  out.push(`   Post: ${j.postType ?? 'No post (service trip only)'}`)
  out.push(`   Property: ${j.propertyType}`)
  if (j.lines.length) {
    out.push('   Install:')
    for (const l of j.lines) out.push(`     - ${dispatchLineText(l)}`)
  }
  if (j.installLocation) out.push(`   Location: ${j.installLocation}`)
  const flags: string[] = []
...
function renderDispatchServiceText(j: DispatchServiceJob, n: number): string {
  const out: string[] = []
  const where = j.address.onFile
    ? `${j.address.line1}, ${j.address.city} ${j.address.state} ${j.address.zip}`
    : 'address not on file with PPI — call agent'
  out.push(`${n}) ${where} — SERVICE TRIP: ${j.type}${j.status === 'completed' ? '   (already marked completed)' : ''}`)
  if (j.mapsUrl) out.push(`   Map: ${j.mapsUrl}`)
  ...
  if (j.notes) out.push(`   Notes: ${j.notes}`)
  if (j.installedHere) {
    out.push(`   What was installed here (${j.installedHere.orderNumber}):`)
    for (const l of j.installedHere.lines) out.push(`     - ${dispatchLineText(l)}`)
  }
  if (j.ridersOnSite.length) out.push(`   Riders on site: ${j.ridersOnSite.join(', ')}`)
  if (j.lockboxesOnSite.length) {
    out.push('   Lockboxes on site:')
    for (const lb of j.lockboxesOnSite) out.push(`     - ${formatExistingLockboxLine(lb)}`)
  }
  return out.join('\n')
}
...
function renderDispatchJobHtml(j: DispatchJob, n: number): string {
  ...
  const agent = (a: { name: string; phone: string | null; company: string | null }) =>
    `${escapeHtml(a.name)}${a.phone ? ` (${escapeHtml(a.phone)})` : ''}${a.company ? ` · ${escapeHtml(a.company)}` : ''}`

  let title: string
  const rows: string[] = []
  if (j.kind === 'order') {
    const addr = `${j.address.line1}, ${j.address.city} ${j.address.state} ${j.address.zip}`
    title = `${n}) <a href="${j.mapsUrl}" style="color: #E84A7A;">${escapeHtml(addr)}</a>${j.isExpedited ? ' <span style="color: #B45309; font-weight: bold;">⚡ EXPEDITED</span>' : ''}${j.status === 'completed' ? ' <span style="color: #666;">(already marked completed)</span>' : ''}`
    rows.push(row('Date', escapeHtml(dispatchDayLabel(j.scheduledDate, 'long'))))
    rows.push(row('Agent', agent(j.agent)))
    if (j.soldBy) rows.push(row('Sold by', `${escapeHtml(j.soldBy.name)}${j.soldBy.phone ? ` (${escapeHtml(j.soldBy.phone)})` : ''}`))
    rows.push(row('Post', escapeHtml(j.postType ?? 'No post (service trip only)')))
    rows.push(row('Property', escapeHtml(j.propertyType)))
    if (j.lines.length) rows.push(row('Install', lines(j.lines)))
    ...
    const addr = j.address.onFile
      ...
      : '<span style="color: #B45309;">address not on file with PPI — call agent</span>'
...
export function renderInstallerDispatchEmail(props: InstallerDispatchEmailProps): {
  ...
  const days = Array.from(new Set(jobs.map(dayOf).filter((d): d is string => !!d)))
  const anyExpedited = jobs.some((j) => j.kind === 'order' && j.isExpedited)
  const dayLabel =
    days.length === 1 && jobs.every((j) => dayOf(j) === days[0])
      ? dispatchDayLabel(days[0], 'short')
      : days.length === 0
        ? 'next available'
        : 'multiple dates'
  const subject = `${anyExpedited ? '⚡ ' : ''}Install schedule — ${dayLabel} (${jobs.length} job${jobs.length === 1 ? '' : 's'})`
  ...
  const hr = '================================================================'
  const heading = days.length === 1 && jobs.every((j) => dayOf(j) === days[0]) ? dispatchDayLabel(days[0], 'long') : 'the selected days'

  const textParts: string[] = []
  // No "To:" line in the body — the mail client shows recipients, and the
  // admin preview is rendered before the final list is typed.
  // The note goes right after the heading; DispatchEmailModal splices the
  // typed note into the preview at this exact spot so preview == sent.
  textParts.push(`Pink Posts — jobs for ${heading}`)
  if (note) textParts.push(`\n----\n${note}\n----`)
  jobs.forEach((j, i) => {
    textParts.push(hr)
    textParts.push(j.kind === 'order' ? renderDispatchOrderText(j, i + 1) : renderDispatchServiceText(j, i + 1))
  })
  textParts.push(hr)
  textParts.push(`Details as of ${stamp} ET — sent by ${sentByName}. Reply to this email if anything is unclear.`)
  const text = textParts.join('\n')

  // Gmail clips HTML bodies past ~102 KB; above 40 jobs the plain-text body
  ...
  const html =
    ...
    <!DOCTYPE html>
    <html>
    <head><meta charset="utf-8"><title>Install schedule</title></head>
    <body style="font-family: 'Poppins', Arial, sans-serif; max-width: 640px; margin: 0 auto; padding: 20px; background-color: #FFF0F3;">
      <div style="text-align: center; margin-bottom: 16px;">
        <h1 style="color: #E84A7A; margin: 0; font-size: 22px;">Pink Posts Installations</h1>
        <p style="color: #666; margin: 6px 0 0;">Jobs for ${escapeHtml(heading)} · ${jobs.length} job${jobs.length === 1 ? '' : 's'}</p>
      </div>
      ${note ? `<div style="background-color: #DBEAFE; border: 1px solid #93C5FD; color: #1E3A8A; padding: 12px 14px; border-radius: 8px; margin: 0 0 14px; white-space: pre-wrap;">${escapeHtml(note)}</div>` : ''}
      ${jobs.map((j, i) => renderDispatchJobHtml(j, i + 1)).join('')}
      <p style="color: #999; font-size: 12px; text-align: center; margin: 16px 0 0;">Details as of ${escapeHtml(stamp)} ET — sent by ${escapeHtml(sentByName)}. Reply to this email if anything is unclear.</p>
    </body>
    </html>`

  const rendered = { subject, text, html }
  // Backstop — throws if any dollar amount survived. The caller treats this
  // as "refuse to send".
...
export async function sendInstallerDispatchEmail(props: InstallerDispatchEmailProps): Promise<{ id: string | null }> {
  ...
  if (r.error) throw new Error(r.error.message || 'Resend rejected the dispatch email')
